...
When documentation work surfaces a significant architectural decision (new framework choice, data model design, auth strategy, API architecture, build tool selection, hosting platform), recommend writing an Architecture Decision Record in `docs/decisions/` rather than burying the rationale in a README or AGENTS.md. ADRs capture *why* a decision was made, what alternatives were considered, and what consequences follow — context that code and config alone cannot convey.

This skill does not create ADRs itself. It routes the user to write one. When an AGENTS.md references ADRs, include a link like `See [docs/decisions/](docs/decisions/) for architectural decision records.` in the Architecture section.
