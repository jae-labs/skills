...
- Detection: alert, SLO burn, traces, logs, metrics, dashboards, customer reports. Verify that any new alert uses the right signals to avoid alert noise and fatigue (e.g., `p99`/`p99.9` latency for high-throughput user experience, `MAX` for resource saturation hotspots, and lag in time instead of message counts for queues/Kafka). Verify that alerts are designed using multi-window multi-burn-rate strategies on error budget ratios where possible, are actionable, and link directly to runbooks.
...
- Observability: useful logs, metrics, traces, dimensions, correlation IDs, service/env naming, root-cause value, actionable alerts, and alert fatigue prevention.
  - Signal selection guidelines:
    - **Latency:** Never use average (`AVG`) latency (masks tail latency) or `MAX` latency (too noisy/subject to transient blips/GC pauses). Prefer `p99` or `p99.9` for high-throughput SLOs/alerts, and reserve `MAX` only for low-throughput, critical operations where percentiles are statistically unstable.
    - **Saturation (CPU, Memory, Connections):** Prefer `MAX` across cluster nodes over `AVG` or percentiles to detect hotspotting and single-instance saturation.
    - **Queues & Kafka:** Avoid alerting on message count/depth alone; alert on lag in time (freshness) since high message volume is acceptable if processed quickly. Consider the rate of lag accumulation (velocity of lag) to alert predictively before queue saturation, but avoid paging on high lag if the backlog is already draining (negative rate of lag).
  - Alert design guidelines:
    - Prefer **Multi-Window Multi-Burn-Rate alerts** based on SLO error budget consumption to avoid paging on transient spikes while responding quickly to serious budget depletion.
    - Calculate error rates as **Ratios** (e.g. `sum(rate(errors))/sum(rate(requests))`) instead of raw counters to handle fluctuating traffic volumes.
    - Ensure every alert is actionable and links directly to a runbook.
